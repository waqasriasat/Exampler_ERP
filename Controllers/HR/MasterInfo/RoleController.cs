...
{
  public class RoleController : PositionController
  {
    ...
    public async Task<IActionResult> ExportToExcel()
    {
      ...
      {
        var worksheet = package.Workbook.Worksheets.Add(_localizer["lbl_RoleType"]);
        worksheet.Cells["A1"].Value = _localizer["lbl_RoleTypeID"];
        worksheet.Cells["B1"].Value = _localizer["lbl_RoleTypeName"];
        ...
        for (int i = 0; i < Rolees.Count; i++)
        {
          ...
          worksheet.Cells[i + 2, 3].Value = Rolees[i].ActiveYNID == 1 ? _localizer["lbl_Yes"] : _localizer["lbl_No"];
        }
        ...
        stream.Position = 0;
        string excelName = _localizer["lbl_RoleType"] +$"-{DateTime.Now.ToString("yyyyMMddHHmmssfff")}.xlsx";
