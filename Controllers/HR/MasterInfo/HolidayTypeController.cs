...
{
  public class HolidayTypeController : PositionController
  {
    ...
    public async Task<IActionResult> ExportToExcel()
    {
      ...
      {
        var worksheet = package.Workbook.Worksheets.Add(_localizer["lbl_HolidayType"]);
        worksheet.Cells["A1"].Value = _localizer["lbl_HolidayTypeID"];
        worksheet.Cells["B1"].Value = _localizer["lbl_HolidayTypeName"];
        ...
        for (int i = 0; i < HolidayTypees.Count; i++)
        {
          ...
          worksheet.Cells[i + 2, 3].Value = HolidayTypees[i].ActiveYNID == 1 ? _localizer["lbl_Yes"] : _localizer["lbl_No"];
        }
        ...
        stream.Position = 0;
        string excelName = _localizer["lbl_HolidayType"] +$"-{DateTime.Now.ToString("yyyyMMddHHmmssfff")}.xlsx";
