...
{
  public class DeductionTypeController : PositionController
  {
    ...
    public async Task<IActionResult> ExportToExcel()
    {
      ...
      {
        var worksheet = package.Workbook.Worksheets.Add(_localizer["lbl_DeductionType"]);
        worksheet.Cells["A1"].Value = _localizer["lbl_DeductionTypeID"];
        ...
        for (int i = 0; i < DeductionTypees.Count; i++)
        {
          ...
          worksheet.Cells[i + 2, 3].Value = DeductionTypees[i].ActiveYNID == 1 ? _localizer["lbl_Yes"] : _localizer["lbl_No"];
        }
        ...
        stream.Position = 0;
        string excelName = _localizer["lbl_DeductionType"] +$"-{DateTime.Now.ToString("yyyyMMddHHmmssfff")}.xlsx";
