...
namespace Exampler_ERP.Controllers.Finance.Transaction
{
  public class PaymentVoucherController : Controller
  {
    ...
    public PaymentVoucherController(AppDBContext appDBContext, IConfiguration configuration, Utils utils, IHubContext<NotificationHub> hubContext)
    {
        ...
        try
        {
          ...
          if (voucherId > 0)
          {
            ...
            else
            {
              ...
              await _appDBContext.SaveChangesAsync();
              TempData["SuccessMessage"] = "Payment Voucher successfully. No process setup found, Payment Voucher Approved.";
              return Json(new { success = true, message = "No process setup found, Payment Voucher Approved." });
            }
          }
          TempData["SuccessMessage"] = "Payment Voucher Created successfully. Continue to the Approval Process Setup for Payment Voucher Approved.";
