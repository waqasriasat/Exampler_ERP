  ...
  public class EmployeeRequestController : Controller
  {
    ...
    public async Task<IActionResult> Index()
    {
      ...
      var EmployeeRequests = await _appDBContext.HR_EmployeeRequests
                                   ...
                                   .OrderByDescending(v => v.EmployeeRequestID)
    ...
    public async Task<IActionResult> Edit(int id)
    {
      ...
      var EmployeeRequests = await _appDBContext.HR_EmployeeRequests
      .Where(v => v.EmployeeRequestID == id)
                                   .Include(c => c.Employee)
                                    .Include(c => c.Settings_EmployeeRequestType)
                                   .FirstOrDefaultAsync();
    ...
    [HttpPost]
    public async Task<IActionResult> Edit(HR_EmployeeRequest EmployeeRequest)
    {
    ...
    public async Task<IActionResult> Create()
    {
      ...
      return PartialView("~/Views/EmployeePortal/Apply/EmployeeRequest/AddEmployeeRequest.cshtml", new HR_EmployeeRequest());
    }

    [HttpPost]
    public async Task<IActionResult> Create(HR_EmployeeRequest EmployeeRequest)
    {
      if (ModelState.IsValid)
      {
        ...
        var EmployeeRequestId = EmployeeRequest.EmployeeRequestID;
        if (EmployeeRequestId > 0)
        {
          ...
          if (processCount > 0)
          {
            var newProcessTypeApproval = new CR_ProcessTypeApproval
            {
              ...
              TransactionID = EmployeeRequest.EmployeeRequestID
            };
          ...
          else
          {
            ...
            EmployeeRequest.ApprovalProcessID = 0;
            _appDBContext.HR_EmployeeRequests.Update(EmployeeRequest);
            await _appDBContext.SaveChangesAsync();
            return Json(new { success = true });
          }
        }
    ...
    {
      var EmployeeRequests = await _appDBContext.HR_EmployeeRequests.FindAsync(id);
      ...
      _appDBContext.HR_EmployeeRequests.Update(EmployeeRequests);
      await _appDBContext.SaveChangesAsync();
    ...
    public async Task<IActionResult> ExportToExcel()
    {
      ...
      using (var package = new ExcelPackage())
      {
        ...
        {
          worksheet.Cells[i + 2, 1].Value = EmployeeRequests[i].EmployeeRequestID;
          ...
          ? ""
          : EmployeeRequestTypesList.FirstOrDefault(g => g.Value == EmployeeRequests[i].EmployeeRequestTypeID.ToString())?.Text;
